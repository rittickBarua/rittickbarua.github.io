...
Most house-price models predict a number and stop. This project asks something more useful and more falsifiable: using only publicly available data, can you identify which current listings are mispriced — and can the resulting tool be interrogated rather than trusted blindly?

## Approach

- **Training data.** 2.18 million Land Registry transactions spanning 1995–2026, deflated to a common reference with the House Price Index.
- **Model.** A stacked ensemble — per-property-type, Optuna-tuned XGBoost base learners feeding a quantile-loss meta-learner — producing an 80% prediction interval for every property rather than a bare point estimate.
...
- An IAAO coefficient of dispersion of 8.6 — the "excellent" band — and a price-related differential near 1.0, indicating no systematic vertical inequity.

**Stack:** Python, XGBoost, Optuna, quantile regression, geospatial analysis, a stacked meta-learner architecture.
